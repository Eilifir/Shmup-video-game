...
#Art from Kenny.nl
import pygame           #imports libraries of pygame & random
import random
from os import path
...
class Player(pygame.sprite.Sprite):                 #defines the player
    def __init__(self):
        ...
        self.lives = 3
        self.hidden = False
        self.hiden_timer = pygame.time.get_ticks()
        self.power = 1
...
class Mob(pygame.sprite.Sprite):
    ...
    def rotate(self):
        now = pygame.time.get_ticks()
        if now - self.last_update > 50:         #if the time now vs the last time when we update is greater than 50ms then
